...
self.addEventListener('message', async (event: MessageEvent<ClangdWorkerInboundMessage>) => {
	...
	try {
		...
		const assetUrls = resolveRuntimeAssetUrls(baseUrl, manifest);
		const jsResponse = await fetch(assetUrls.clangdJs);
		if (!jsResponse.ok) {
		...
		const wasmResponse = await fetch(assetUrls.clangdWasm);
		if (!wasmResponse.ok) {
