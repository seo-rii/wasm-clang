...
describe('GNU compatibility headers', () => {
	it('provides PBDS tree and order-statistics headers under the libc++ include root', () => {
		...
		expect(GCC_COMPATIBILITY_HEADERS[1]?.contents).toContain(
			'class tree<Key, null_type, Cmp_Fn, Tag, Node_Update, Allocator>'
		);
		expect(GCC_COMPATIBILITY_HEADERS[1]?.contents).toContain('find_by_order');
		expect(GCC_COMPATIBILITY_HEADERS[1]?.contents).toContain('order_of_key');
		expect(GCC_COMPATIBILITY_HEADERS[1]?.contents).toContain('gp_hash_table');
	});
